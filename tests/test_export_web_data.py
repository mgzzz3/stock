...
class CombineWebDataTests(unittest.TestCase):
    def test_predictions_annotate_signals_and_discard_unmatched_picks(self):
        ...
        self.assertEqual(list(combined["ts_code"]), ["000002.SZ", "000001.SZ"])
        ...
        self.assertNotIn("000003.SZ", predicted.index)
        self.assertTrue(pd.isna(predicted.loc["000001.SZ", "prob_up"]))
        self.assertEqual(len(combined), 2)

    def test_prediction_file_without_b1_signal_file_exports_no_rows(self):
        with tempfile.TemporaryDirectory() as temp_dir:
