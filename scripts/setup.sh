...
echo "=== 1/6  stock_basic (全量股票列表) ==="
uv run python -m ingest.stock_basic

echo
echo "=== 2/6  trade_cal (交易日历) ==="
uv run python -m ingest.trade_cal

echo
echo "=== 3/6  backfill (全量日线, 默认近5年) ==="
uv run python -m ingest.backfill

echo
echo "=== 4/6  fundamentals (价值策略点时年报) ==="
uv run python -m ingest.fundamentals --history

echo
echo "=== 5/6  indicators 全量计算 ==="
uv run python -m indicators.kdj
uv run python -m indicators.zhixing

echo
echo "=== 6/6  strategy + export ==="
